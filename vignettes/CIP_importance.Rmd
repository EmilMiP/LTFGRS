---
title: "Reflections on the use of population representative CIPs"
author: "Emil Pedersen"
date: "`r Sys.Date()`"
output: html_document
vignette: >
  %\VignetteIndexEntry{Reflections on the use of population representative CIPs}
...
This vignette is intended to serve as cautionary tale of why population representative CIPs are preferred when estimating genetic liabilities. The true impact of using CIPs that are not population representative is currently unknown.
...
A long history of calculating CIPs in population representative samples already exist, such as with [Carsten Pedersen et al.](https://doi.org/10.1001/jamapsychiatry.2014.16) in the Danish national registers or in Finland with the [Riesteys](https://risteys.finngen.fi/) project based on both FinnGen and the Finnish national health registries.

Attempts to make CIPs calculated on an ascertained sample more representative of the target population have been made, such as with inverse probability weighting in iPSYCH ([Theresa Wimberley et al.](https://doi.org/10.1186/s12982-025-00888-w)), or attempting to quantifying the participation bias in UK Biobank ([Tabea Schoeler et al.](https://doi.org/10.1038/s41562-023-01579-9)). 
...
We simulate a family consisting of two parents, one sibling, and the proband.
...
We will use this small family to generate a suiable covariance matrix, that will be used to generate liabilities for the family members based on the liability threshold model.
...
Next, we will replicate the small family $nsim$ times with an iterator for each family:
...
### Mock phenotypes
With each family replicated $nsim$ times, we can now perform $nsim$ draws from a multivariate normal distribution with a suitable covariance matrix to generate liabilities for each individual in each family. We use `convert_liability_to_aoo()` to link the full liability of an individual to the age of onset. We use a logistic function to link the two.
```{r}
...
When CIPs are calculated in a population representative sample, they will more closely reflect the true risk an individual has already lived through.
If we assume the population is fully observed, i.e. no censoring, then we can use a naïve estimate of the CIP. If censoring is present in a sample, a more flexible approach must be used Kaplan-Meier or, if competing events are likely to be present, Aalen-Johansen. 
```{r}
...
If instead a CIP from a population representative sample is not available. It is possible to calculate the CIP from the sampled data only, however, this is likely to not accurately capture the true risk an individual has lived through relative to the population that they originate from. Some samples, like iPSYCH, will be heavily over-represent the cases of the iPSYCH phentoypes, while UK Biobank is known to skew towards more women than men and be healthier than the general population. These sample biases can be avoided with population representative CIPs.
...
```{r, out.width="90%", fig.alt = "Plot of the two different CIPs as a function of age."}
plot_data_CIPs = 
  bind_rows(
...
Then we build the population graph and family graphs that automatically identifies all family members up to degree $1$:
...
Finally, we can calculate the FGRS using the population representative CIP thresholds:
```{r}
...
```{r, out.width="90%", fig.alt = "Plot of the true genetic liability against the estimated FGRS using both CIPs."}
plot_data = bind_rows(
...
The correlation between the true genetic liability and the estimated FGRS is higher when using population representative CIPs compared to over-ascertained CIPs. 
Since this is simulations, we are able to quantify the difference, but in real data applications, the true genetic liability is unknown, and therefore it is not straight forward to quantify the impact of using non-population representative CIPs. Since both correlations are high, the difference may not be obvious, but it is still present and **may** impact downstream analyses or simply reduce power slightly. The true impact is currently unknown.

...
graph may contain millions of individuals. To illustrate all the
required steps, we will recalculate the population graph, but add sex as
a node attribute.
...
#> IGRAPH 67e0742 DN-- 8 17 -- 
#> + attr: name (v/c), sex (v/n)
#> + edges from 67e0742 (vertex names):
...
offers a way to censor future events on a family basis, by censoring all
events that happen after the end of follow up of the proband
(`indiv_eof`).
...
The function
[`familywise_censoring()`](https://emilmip.github.io/LTFGRS/reference/familywise_censoring.md)
will return a tibble with the following additional columns:
...
Once the `status`, `aod`, and `age` are known, we can assign thresholds
to each family and their family members. Notably, this means that if an
individual appears in multiple families, e.g. as a proband and as a
relative to a different proband, that individual may have multiple
(potentially different) thresholds assigned to them. The use of `fid`
and `pid` helps ensure that each individual can still be uniquely
identified. Due to data privacy, it is possible to encounter CIPs values
that are only provided at set values, e.g. a CIP value for each whole
year by birth year and sex, such as what is shown in with the `CIP`
object. However, the observed ages (or age of diagnosis) are typically
not integer values. This means we may need to approximate the CIP values
...
family graphs. We can attach the thresholds from `fam_thrs` to
...
by setting the `upper` and `lower` values to $\infty$ and $- \infty$
(non-informative values) and all other values to be `NA` with the
argument `proband_cols_to_censor = c("lower", "upper")`. If `K_i` or
`K_pop` are in `proband_cols_to_censor`, they will be set to NA, which
are uninformative in the PA algorithm and will not be problematic.
...
ltfgrs_input = familywise_attach_attributes(family_graphs = family_graphs,
                                            ...
                                            proband_cols_to_censor = c("lower", "upper"))
...
in the column name. Noteably, the uncertainties with the Gibbs and PA
